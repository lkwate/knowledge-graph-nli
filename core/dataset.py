...
class NLIDataset(Dataset):
    ...
    def get_instances(self, df):
        ...
        if self.group_by_size:
            sorted_criterion = lambda x: len(
                x[1][self.text_columns[0]].split()
            )  # + len(x[1][self.text_columns[1]].split())
        ...
        for row in rows:
            ...
            yield self.to_tensor(output1), self.to_tensor(output2), float(
                self.label_factory[label]
            )
